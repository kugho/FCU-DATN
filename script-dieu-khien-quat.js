...
// ????????????????????????????????????????????????????????????????
database.ref("Monitor/concac/data").on("value", function(snapshot){
...
database.ref("Monitor/CPS-A/data").on("value", function(snapshot){
  ...
  if (pres > 35 ) document.getElementById("call").style.display = "block";
  else document.getElementById("call").style.display = "none";
})
database.ref("Control/STATUS/data").on("value", function(snapshot){
  var mode_ther = snapshot.val();
  if (mode_ther == 0 ) 
    {
      // document.getElementById("call-thermostat").disabled = true;
    }
})
database.ref("Control/VIRTUAL OV VALVE/data").on("value", function(snapshot){
...
// database.ref("Control/O VALUE/data").on("value", function(snapshot){
//   var monitor_frq = snapshot.val();
//   document.getElementById("myRange").innerHTML = monitor_frq;
// })
// get CURRENT from firebase (auto update when data change)
database.ref("Monitor/CURRENT/data").on("value", function(snapshot){
  var current_fan = snapshot.val();
  document.getElementById("current").innerHTML = current_fan/10;
})
...
database.ref("Monitor/POWER/data").on("value", function(snapshot){
 ...
 database.ref("Monitor/TEMP1/data").on("value", function(snapshot){
  ...
  document.getElementById("themotat").innerHTML = thermos_temp;
}) 
  ...
  if (onoffthermostat==0)
    {
      ...
      document.getElementById("call-thermostat-low").style.display = "none";
    document.getElementById("call-thermostat-medium").style.display = "none";
    document.getElementById("call-thermostat-high").style.display = "none";
    document.getElementById("call-thermostat-auto").style.display = "none";
    }
else
  {
...
// get CURRENT from firebase (auto update when data change)
database.ref("Control/ON OFF THERMOSTAT/data").on("value", function(snapshot){
  var on_off = snapshot.val();
  
  if (on_off==1)
    {
      database.ref("Control/STATUS/data").on("value", function(snapshot1){
        var status = snapshot1.val();
        if (status == 0)
          {
            database.ref("Control/VIRTUAL RUN CM").update({"data" : 2});
            document.getElementById("call-thermostat-low").style.display = "block";
            document.getElementById("call-thermostat-medium").style.display = "none";
            document.getElementById("call-thermostat-high").style.display = "none";
            document.getElementById("call-thermostat-auto").style.display = "none";
          }
        else if (status == 1)
          {
            database.ref("Control/VIRTUAL RUN CM").update({"data" : 2});
            document.getElementById("call-thermostat-low").style.display = "none";
            document.getElementById("call-thermostat-medium").style.display = "block";
            document.getElementById("call-thermostat-high").style.display = "none";
            document.getElementById("call-thermostat-auto").style.display = "none";
          }
          else if (status == 2)
            {
              ...
              document.getElementById("call-thermostat-low").style.display = "none";
              document.getElementById("call-thermostat-medium").style.display = "none";
              document.getElementById("call-thermostat-high").style.display = "block";
              document.getElementById("call-thermostat-auto").style.display = "none";
            }
            else if (status == 3)
              {
                ...
                document.getElementById("call-thermostat-medium").style.display = "none";
                document.getElementById("call-thermostat-high").style.display = "none";
                document.getElementById("call-thermostat-auto").style.display = "block";
              }
...
function toggleImage() {
  // Get the checkbox
  var checkBox = document.getElementById("imageCheckbox-thermostat");
